...
SITE_ROOT = 'http://dati.emilia-romagna.it'
...
def _process_service_url(attr_value_dom):
	return _func_name_attr_value(attr_value_dom)
...
attribute_actions = {\
	...
	'url del servizio'                    : _process_service_url,
	...
	for attribute_row in dataset_page_dom.cssselect('tr'):
		...
		if len(attr_name) == 1:
			...
			try:
				...
				raise Exception('Error while processing attribute [%s]: %s' % (attr_name, repr(e)) )
...
def process_search_page(search_page):
	...
	try:
		...
		catalog_size = int(last_page_re.findall(last_page_js)[0])
	except Exception as e:
		raise Exception('Error while retrieving catalog size: ' + repr(e) )
	process_catalog(catalog_size)
