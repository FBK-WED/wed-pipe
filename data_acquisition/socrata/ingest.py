...
url_id_extractor_re = re.compile('^.+/views/([0-9a-z]{4}-[0-9a-z]{4})(/rows)?\.json$')
...
class SocrataPostGresImporter():
	...
	def describe(self, out=sys.stdout):
		...
		for result in datasets['results']:
			...
			out.write( 'ATTRIBUTION: %s %s\n' % (view['attribution'], view['attributionLink'] if view.has_key('attributionLink') else '') )
			out.write( 'DESCRIPTION: %s\n'    % view['description'] if view.has_key('description') else '' )
			out.write( 'TAGS: %s\n'           % view['tags'] if view.has_key('tags') else '' )
			for col in view['columns']:
				out.write( '\t\t %s (%s) - %s\n' % (col['fieldName'], col['dataTypeName'], col['name']) )

	# 'url', 'download', 'name', 'curator', 'license', 'description', 'tags', 'bouding_box'
	def describe_json(self):
		...
		for result in datasets['results']:
			view = result['view']
			json_meta = {}
			json_meta['url']         = '%s/api/views/%s.json' % (self._dataset_api.host, view['id'])
			json_meta['download']    = self.__download_url(view, 'json')
			json_meta['name']        = view['name']
			json_meta['curator']     = view['attribution']
			json_meta['license']     = view['license']['termsLink']
			json_meta['description'] = view['description'] if view.has_key('description') else ''
			json_meta['tags']        = view['tags'] if view.has_key('tags') else ''
	...
	def get_sql_create_table(self, view):
		sql = 'CREATE TABLE %s (' % self.__table_name(view)
		fields = []
		for col in view['columns']:
			fields.append( '%s %s' %  (self.__rewrite_name(col['fieldName']), self.__rewrite_datatype(col['dataTypeName']) ) )
		sql += ', '.join(fields) + ')'
		return sql
	...
	def load_table(self, view):
		...
		if exit_code != 0: raise Exception('Error while ingesting CSV with command [' + command + ']')
	...
	def ingest_view_from_url(self, view_url):
		...
		if not id_match: raise Exception('Invalid view_url: ' + view_url)
	...
	def __new_dataset(self):
		...
		return Dataset( api_conf['host'], api_conf['user'], api_conf['passwd'], api_conf['token'] )
	...
	def __execute_sql_command(self, sql, verify=True):
		...
		exit_code = subprocess.call(command, shell=True)
		if verify and exit_code != 0: raise Exception('Error while executing SQL command [' + command + ']')
...
class Dataset(object):
	...
	def metadata(self):
		return (self._request("/views/%s.json" % self.id))
	...
	def _request(self, service, content_type='application/json'):
		...
		if content is not None and len(content) > 0:
			...
			if hasattr(response_parsed, 'has_key') and response_parsed.has_key('error') and response_parsed['error'] == True:
...
if __name__ == '__main__':
	...
	parser.add_argument('conf'           , type=str, help='Tool JSON configuration')
	parser.add_argument('--describe'     , action='store_true', help='Show human readable description for the API.')
	parser.add_argument('--describejson' , action='store_true', help='Show JSON metadata for the API compatible with the SW Meta table.')
	parser.add_argument('--importer'     , action='store_true', help='Import all portal data into PostGres.')
	parser.add_argument('--urlimporter'  , action='store_true', help='Import specified dataset URL into PostGres.')
	parser.add_argument('--url'          , type=str,            help='The urlimporter target URL')
	...
	if args.describe:
		...
		if not args.url: raise Exception('--urlimporter requires --url option')
